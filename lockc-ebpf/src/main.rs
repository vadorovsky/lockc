...
use aya_bpf::{
    ...
    macros::{btf_tracepoint, fexit, lsm},
    programs::{BtfTracePointContext, FExitContext, LsmContext},
};
...
#[inline]
unsafe fn get_policy_level() -> Result<ContainerPolicyLevel, i32> {
    // let pid = bpf_get_current_pid_tgid() as u32;
    let pid = (bpf_get_current_pid_tgid() >> 32) as i32;
    let process_o = PROCESSES.get(&pid);
    match process_o {
        Some(process) => {
            let container_o = CONTAINERS.get(&process.container_id);
            match container_o {
                Some(container) => Ok(container.policy_level),
                None => Err(-2),
            }
        }
        None => Ok(ContainerPolicyLevel::NotFound),
    }
}
