...
int BPF_PROG(do_exit, long code)
{
	...
	bpf_printk("process %d is exiting\n", pid);
...
int BPF_KPROBE(add_container, int *retp, u32 container_id, pid_t pid, int policy)
{
	...
	if (err < 0) {
		bpf_printk("adding container: containers: error\n");
		ret = err;
		goto out;
	}
	...
	if (err < 0) {
		bpf_printk("adding container: processes: error\n");
		ret = err;
		goto out;
	}
...
int BPF_KPROBE(delete_container, int *retp, u32 container_id)
{
	...
	if (err < 0) {
		bpf_printk("deleting container: error\n");
		ret = err;
		goto out;
	}
	if (cb.err < 0) {
...
int BPF_KPROBE(add_process, int *retp, u32 container_id, pid_t pid)
{
	...
	err = bpf_map_update_elem(&processes, &pid, &p, 0);
	if (err < 0) {
		bpf_printk("adding process: error\n");
		ret = err;
		goto out;
	}
