...
int BPF_PROG(mount_audit, const char *dev_name, const struct path *path,
  ...
  if (unlikely(bpf_probe_read_kernel_str(&type_safe, MOUNT_TYPE_LEN, type) <
               0)) {
  ...
  if (unlikely(bpf_probe_read_kernel_str(&dev_name_safe, PATH_LEN, dev_name) <
               0)) {
  ...
  switch (policy_level) {
  case POLICY_LEVEL_RESTRICTED: {
    struct path_mount_key key;
    __builtin_memcpy(key.lpm_key.data, &dev_name_safe, sizeof(key.path));
    key.lpm_key.prefixlen = PATH_LEN;
    u32 *val = bpf_map_lookup_elem(&allowed_paths_mount_restricted, &key);
    if (val)
      goto out;
    break;
  }
  case POLICY_LEVEL_BASELINE: {
    ...
    __builtin_memcpy(key.lpm_key.data, &dev_name_safe, sizeof(key.path));
    key.lpm_key.prefixlen = PATH_LEN;
    u32 *val = bpf_map_lookup_elem(&allowed_paths_mount_baseline, &key);
    if (val)
      goto out;
    break;
  }
...
int BPF_PROG(open_audit, struct file *file, int ret_prev) {
  ...
  switch (policy_level) {
  case POLICY_LEVEL_RESTRICTED:
    bpf_for_each_map_elem(&denied_paths_access_restricted, check_paths, &cb, 0);
    if (cb.found) {
    ...
    cb.found = false;
    bpf_for_each_map_elem(&allowed_paths_access_restricted, check_paths, &cb,
                          0);
    if (cb.found) {
      bpf_printk("open: restricted: allow\n");
      goto out;
    ...
    break;
  case POLICY_LEVEL_BASELINE:
    bpf_for_each_map_elem(&denied_paths_access_baseline, check_paths, &cb, 0);
    if (cb.found) {
    ...
    cb.found = false;
    bpf_for_each_map_elem(&allowed_paths_access_baseline, check_paths, &cb, 0);
    if (cb.found) {
